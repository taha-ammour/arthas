...
public class ProfilerCommand extends AnnotatedCommand {
    ...
    private void processStart(final AsyncProfiler asyncProfiler, final CommandProcess process) throws IOException {
    ...
    private void processExecuteAction(AsyncProfiler asyncProfiler, CommandProcess process) throws IllegalArgumentException, IOException {
        if (actionArg == null) {
    ...
    private void processDumpCollapsed(AsyncProfiler asyncProfiler, CommandProcess process) {
        if (actionArg == null) {
            actionArg = "TOTAL";
        }
    ...
    private void processDumpFlat(AsyncProfiler asyncProfiler, CommandProcess process) {
        int maxMethods = 0;
        if (actionArg != null) {
    ...
    private void processDumpTraces(AsyncProfiler asyncProfiler, CommandProcess process) {
        int maxTraces = 0;
        if (actionArg != null) {
    ...
    public void process(final CommandProcess process) {
        try {
            ...
            if (ProfilerAction.execute.equals(profilerAction)) {
                ...
                processDumpCollapsed(asyncProfiler, process);
            } else if (ProfilerAction.dumpFlat.equals(profilerAction)) {
                processDumpFlat(asyncProfiler, process);
            } else if (ProfilerAction.dumpTraces.equals(profilerAction)) {
                processDumpTraces(asyncProfiler, process);
    ...
    private String generateOutputFilePath() throws IOException {
        ...
        if (outputPath != null) {
            ...
            return File.createTempFile("arthas-output", "." + fileExt).getAbsolutePath();
        }
    }
